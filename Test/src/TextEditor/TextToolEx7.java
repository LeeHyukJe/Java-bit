...
public class TextToolEx7 extends Frame implements WindowListener {
	...
	String[] btnName = { "Undo", "짝수줄삭제", "문자삭제", "trim", "빈줄 삭제", "접두사 추가", "substring", "substring2" };
	...
	private void registerEventHandler() {
		...
		btn[n++].addActionListener(new ActionListener() {// 접두사 접미사 삭제
			public void actionPerformed(ActionEvent ae) {
				...
				while (sc.hasNextLine()) {
					temp=sc.nextLine();
					int first=temp.indexOf(tfParam1.getText());
					int last=temp.indexOf(tfParam2.getText());
					temp=temp.substring(first+2,last);
					sb.append(temp).append(CR_LF);
			}
				ta.setText(sb.toString());
			}
	...
	public static void main(String[] args) {
		...
		TextToolEx7 ex7 = new TextToolEx7("TextTool");
		ex7.show();
	}
